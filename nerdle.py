...
def submit_triggered():
    ...
    for entry in entries[row_index + 1]:
        ...
        if '' == char:
            ...
            return
        elif not (char.isdigit() or char in operators):
            messagebox.showerror("Error", "Invalid!\nYou need to enter a number or valid math operator")
            return
        else:
...
entries = [
            [Entry(entries_frame, width=2, state='disabled', font=('Arial Rounded MT Bold', 28), justify='center') for x in range(8)]
                for y in range(6)
            ]
